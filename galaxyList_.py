...
with fits.open('NEDGalaxyCatalog/GIT_Observable_catalog.fits') as hdu:
...
def get_90_Loc_Area(ipix, prob, pixarea, distmu, distsigma, distnorm, center, filename, catNew):
    ...
    dp_dr=cat['Dist']**2 * distnorm[ipix] * norm(distmu[ipix],distsigma[ipix]).pdf(cat['Dist'])
    catNew['dp_dr'] = dp_dr
...
parser.add_argument("--plot", help = "If we need plots")
...
for i, inj_file in enumerate(injections[0:1]):
    ...
    ipix = hp.ang2pix(nside, catNew['_RAJ2000'], catNew['_DEJ2000'], lonlat=True)
    ...
    if non_zero_mass > 0: 
        ...
        if event_plot_needed=='yes':
            ...
            axi[0][1].set_title("Top {} galaxies in 90% credible volume".format(galToShow), fontsize=15)
...
catMass_pandas.to_csv('galaxy_csv/csvGal_probMass.csv')
catMass.sort('Mstar', reverse=True)
catMass_pandas = catMass.to_pandas()
catMass_pandas.to_csv('galaxy_csv/csvGal_Mass.csv')
